...
export const NewUser = async(req: Request, res: Response): Promise<void> =>{
    ...
    if (!name || !surname || !email || !password || !rol) {
        res.status(400).json({ msg: 'Faltan datos obligatorios en el registro del usuario' });
        return;
    }
    //else{   //Si el user no existe, se permite el ingreso a la db
        try{
            ...
            const newUser = await User.create({ //Incresamos un nuevo usuario a la db
                name: name,
                surname: surname,
                email: email,
            ...
            //Mensaje de exito
            res.status(201).json({ msg: `User ${email} creado exitosamente`, user: newUser });
        }
        catch(error){
